...
export function getPokemonInSet(availableIds: Set<number>): PokemonSpecies[] {
  return POKEMON_SPECIES.filter(p => availableIds.has(p.id));
}
...
export const POKEMON_SPECIES: PokemonSpecies[] = [
  ...
  { id:  592, name: 'Frillish Male', types: ['Water', 'Ghost'] },
  { id:  593, name: 'Jellicent Male', types: ['Water', 'Ghost'] },
  ...
  { id:  668, name: 'Pyroar Male', types: ['Fire', 'Normal'] },
  ...
  { id:  978, name: 'Tatsugiri Curly', types: ['Dragon', 'Water'] },
...
export function getPokemonForTypes(types: PokemonType[], availableIds?: Set<number>): PokemonSpecies[] {
  const all = POKEMON_BY_TYPE_COMBO[comboKey(types)] ?? [];
  if (!availableIds) return all;
  return all.filter(p => availableIds.has(p.id));
}
...
// Search Pokémon by name, optionally restricted to a specific game's available Pokémon
export function searchPokemon(query: string, availableIds?: Set<number>): PokemonSpecies[] {
  const q = query.toLowerCase().trim();
  if (q.length < 2) return [];
  const pool = availableIds ? POKEMON_SPECIES.filter(p => availableIds.has(p.id)) : POKEMON_SPECIES;
